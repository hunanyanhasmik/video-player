...
//         li.innerHTML = `
//             <span id="play" onclick="playVideo('${video.filePath}')">${video.title}</span>
...
//     videoPlayer.src = `http://localhost:3000/${filePath}`;
...
async function fetchVideos() {
    ...
    videos.forEach(video => {
        const li = document.createElement("li");
        li.innerHTML = `
            <span class="video-item" data-id="${video.filePath}">${video.title}</span>
    ...
    // Attach event listeners to each video item, edit, and delete buttons
    ...
    document.querySelectorAll('.edit-btn').forEach(button => {
        button.addEventListener('click', (e) => {
            const videoId = e.target.getAttribute('data-id');
            editVideo(videoId);
            e.stopPropagation(); // Prevent the playVideo function from being triggered
        });
    });
